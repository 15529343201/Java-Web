...
public class ActionMappingManager {
	...
	// 根据请求路径名称,返回Action的映射对象
	 //actionName 当前请求路径
	// 返回配置文件中代表action节点的ActionMapping对象
	...
	private void init(){
		...
		try{
			...
			for(Element ele_action:listAction){
				...
				ActionMapping actionMapping=new ActionMapping();
				actionMapping.setName(ele_package.attributeValue("name"));
				actionMapping.setClassName(ele_package.attributeValue("class"));
				actionMapping.setMethod(ele_package.attributeValue("method"));
				//6.2封装当前action节点下所有的结果视图
				Map<String, Result> results = new HashMap<String,Result>();
				//得到当前action节点下所有的result节点
				Iterator<Element> it=ele_action.elementIterator("result");
				while(it.hasNext()){
					//当前迭代的每一个元素都是<result...>
					Element ele_result=it.next();
